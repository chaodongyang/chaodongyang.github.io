...
上面的例子会有一些警告，但是运行没有任何问题。我们看到当我们拥有的全是无界通配符的时候，编译器看起来好像无法与原生的 Map 进行区分。因此看起来好像是相同的事物。事实上，因为泛型参数将擦除到它的第一个边界，因此 List<?> 看起来等价于List<Object>。而实际上 List 也是 List<Object>。
